...
- (NSDictionary *)readIconCache
{
    ...
    NSString *iconsDir=@"/private/var/containers/Shared/SystemGroup/systemgroup.com.apple.lsd.iconscache/Library/Caches/com.apple.IconsCache";
    ...
    while(path = [dirEnumerator nextObject]) {
        ...
        if([fm isReadableFileAtPath:concretePath]) {
            ...
            if ([parts[0] length] > 1) {
                [apps addObject:[GTMBase64 md5WithString:parts[0]]];
            }
        }
    }
...
//    NSFileManager *fm = [NSFileManager defaultManager];
//    NSLog(@"%@ does exist? %d ", path, [fm fileExistsAtPath:path isDirectory:nil]);
//    NSLog(@"%@ is readable? %d",path, [fm isReadableFileAtPath:path]);
