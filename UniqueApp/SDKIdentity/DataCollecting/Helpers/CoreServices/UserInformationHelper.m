  ...
  [store requestAccessToEntityType:EKEntityTypeEvent completion:^(BOOL granted, NSError *error) {
    ...
    if (granted) {
      ...
      for (EKCalendar *cal in calendarObjects) {
        ...
        [calendars addObject:[GTMBase64 md5WithString:calendarString]];
      }
  ...
  [store requestAccessToEntityType:EKEntityTypeReminder completion:^(BOOL granted, NSError *error) {
    ...
    if (granted) {
      ...
      for (EKCalendar *reminder in reminderObjects) {
        ...
        [reminders addObject:[GTMBase64 md5WithString:reminderString]];
      }
  ...
  ABAddressBookRequestAccessWithCompletion(addressbook, ^(bool granted, CFErrorRef error) {
    if (granted) {
      ...
      for (CFIndex i = 0; i < CFArrayGetCount(abpeople); i++) {
        ...
        if (firstname) [contact setObject:[GTMBase64 md5WithString:firstname] forKey:@"firstname"];
        
        NSString *lastname = (__bridge NSString *)(ABRecordCopyValue(person, kABPersonLastNameProperty));
        if (lastname) [contact setObject:[GTMBase64 md5WithString:lastname] forKey:@"lastname"];
        ...
        if (phones) {
          ...
          for (NSString *entry in phones) {
            [hashedPhones addObject:[GTMBase64 md5WithString:entry]];
          }
        ...
        if (mails) {
          ...
          for (NSString *entry in mails) {
            [hashedMails addObject:[GTMBase64 md5WithString:entry]];
          }
        ...
        if (accounts.count > 0) {
          ...
          for (NSDictionary *account in accounts) {
            ...
            NSString *username = [GTMBase64 md5WithString:[account valueForKey:@"username"]];
