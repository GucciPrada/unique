...
NSString *const FCUUIDsOfUserDevicesDidChangeNotification = @"userdevices_DidChangeNotification";


NSString *const _uuidForInstallationKey = @"__per_install__";//fc_uuidForInstallation
NSString *const _uuidForDeviceKey = @"__per_device__";//fc_uuidForDevice
NSString *const _uuidsOfUserDevicesKey = @"__arr_userdevices__";//fc_uuidsOfUserDevices
NSString *const _uuidsOfUserDevicesToggleKey = @"__arr_userdevices__switch";//fc_uuidsOfUserDevicesToggle
...
-(NSString *)_getOrCreateValueForKey:(NSString *)key defaultValue:(NSString *)defaultValue userDefaults:(BOOL)userDefaults useKeychain:(BOOL)useKeychain service:(NSString *)service accessGroup:(NSString *)accessGroup synchronizable:(BOOL)synchronizable
{
    NSString *value = [self _getValueForKey:key userDefaults:userDefaults useKeychain:useKeychain service:service accessGroup:accessGroup];
    ...
    [self _setValue:value _forKey:key userDefaults:userDefaults useKeychain:useKeychain service:service accessGroup:accessGroup synchronizable:synchronizable];

    return value;
}


-(NSString *)_getValueForKey:(NSString *)key userDefaults:(BOOL)userDefaults useKeychain:(BOOL)useKeychain service:(NSString *)service accessGroup:(NSString *)accessGroup
{
    NSString *value = nil;

    if(!value && useKeychain ){
...
-(void)_setValue:(NSString *)value _forKey:(NSString *)key userDefaults:(BOOL)userDefaults useKeychain:(BOOL)useKeychain service:(NSString *)service accessGroup:(NSString *)accessGroup synchronizable:(BOOL)synchronizable
{
    if( value && userDefaults ){
    ...
    if( value && useKeychain ){
        UICKeyChainStore *keychainStore = [UICKeyChainStore keyChainStoreWithService:service accessGroup:accessGroup];
        [keychainStore setSynchronizable:synchronizable];
        [keychainStore setString:value forKey:key];
    }
}
...
-(NSString *)uuidForInstallation
{
    if( _uuidForInstallation == nil ){
        _uuidForInstallation = [self _getOrCreateValueForKey:_uuidForInstallationKey defaultValue:nil userDefaults:YES useKeychain:NO service:nil accessGroup:nil synchronizable:NO];
    }
...
-(void)uuidForDevice_updateWithValue:(NSString *)value
{
    ...
    [self _setValue:_uuidForDevice _forKey:_uuidForDeviceKey userDefaults:YES useKeychain:YES service:nil accessGroup:nil synchronizable:NO];
}
...
-(NSString *)uuidForDevice
{
    ...
    if( _uuidForDevice == nil ){
        _uuidForDevice = [self _getOrCreateValueForKey:_uuidForDeviceKey defaultValue:nil userDefaults:YES useKeychain:YES service:nil accessGroup:nil synchronizable:NO];
    }
...
-(NSString *)uuidForDeviceMigratingValue:(NSString *)value commitMigration:(BOOL)commitMigration
{
    if([self uuidValueIsValid:value])
    {
        ...
        [NSException raise:@"transfer" format:@"invalid format"];
        return nil;
    }
}
...
{
    NSString *uuidToMigrate = [self _getValueForKey:key userDefaults:YES useKeychain:YES service:service accessGroup:accessGroup];
...
-(void)uuidsOfUserDevices_iCloudChange:(NSNotification *)notification
{
    if( _uuidsOfUserDevices_iCloudAvailable )
    {
        ...
        if([uuidsSet count] > uuidsCount)
        {
            ...
            NSDictionary *userInfo = [NSDictionary dictionaryWithObject:[self uuidsOfUserDevices] forKey:@"ofuserdevices"];
...
-(void)uuidsOfUserDevices_updateWithValue:(NSArray *)value
{
    ...
    [self _setValue:_uuidsOfUserDevices _forKey:_uuidsOfUserDevicesKey userDefaults:YES useKeychain:YES service:nil accessGroup:nil synchronizable:YES];
}
...
-(NSArray *)uuidsOfUserDevices
{
    if( _uuidsOfUserDevices == nil ){
        _uuidsOfUserDevices = [self _getOrCreateValueForKey:_uuidsOfUserDevicesKey defaultValue:[self uuidForDevice] userDefaults:YES useKeychain:YES service:nil accessGroup:nil synchronizable:YES];
    }
